...
{$ENDIF}
  ...
  public
    ...
    function insertAt : Boolean;
    function insertIn(_start : DIterator) : Boolean;
...
{$ENDIF}
  ...
  //
  ////////////////////////////////////////////////////////////////////
  procedure _remove(container : DContainer; const obj : DObject); {$IFDEF SUPPORTS_INLINE} inline; {$ENDIF}
  ...
  procedure remove(container : DContainer; const objs : array of const);

  {** Removes all instances of each item in objs in the range. }
  procedure removeIn(_start, _end : DIterator; const objs : array of const);

  {** Removes all instances of each item in objs from the container, copying
  the removed items to dest. }
  procedure removeTo(container : DContainer; dest : DIterator; const objs : array of const);

  {** Removes all instances of each item in objs from the range, copying
  the removed items to dest. }
  procedure removeInTo(_start, _end, dest : DIterator; const objs : array of const);

  {** Copies the source container to the destination, except for the items
  matching objs. }
  procedure removeCopy(source, destination : DContainer; const objs : array of const);

  {** Copies the source container to a destination iterator, except for the items
  matching objs. }
  procedure removeCopyTo(source : DContainer; dest : DIterator; const objs : array of const);

  {** Copies the source range to the destination, except for the items
  matching objs. }
  procedure removeCopyIn(_start, _end, dest : DIterator; const objs : array of const);
  ...
  procedure InitDObject(out obj : DObject); {$IFDEF SUPPORTS_INLINE} inline; {$ENDIF}

  {** Creates a copy of the DObject. }
  procedure CopyDObject(const source : DObject; out dest : DObject);

  {** Moves a DObject; the original is left empty. }
  procedure MoveDObject(var source: DObject; out dest : DObject); {$IFDEF SUPPORTS_INLINE} inline; {$ENDIF}
...
type
  ...
  public
    ...
    constructor Create(elementSize : Integer);
  end;
...
constructor TPool.Create(elementSize : Integer);
begin
  FSize := elementSize;
  inherited Create;
end;
...
function JenkinsHashWrapper(const buffer; length : Integer) : Integer;
begin
...
function specialTypes(VT: Integer): Boolean;
begin
  Result := False;
  case VT of
    {$IFDEF USELONGWORD}
...
procedure InitDObject(out obj : DObject);
begin
  obj.VType := vtInteger;
  obj.VInteger := 0;
end;
...
procedure ClearDObject(var obj : DObject);
begin
  if SpecialTypes(obj.VType) then
    begin
      case obj.VType of
        vtString:
          FreeMem(obj.VString);
...
procedure InitDObjects(var na: DObjectArray);
var i : Integer;
begin
  for i := Low(na) to High(na) do
    InitDObject(na[i]);
end;

procedure ClearDObjects(var na: DObjectArray);
var i : Integer;
begin
  for i := Low(na) to High(na) do
    ClearDObject(na[i]);
...
procedure MoveDObject(var source: DObject; out dest : DObject);
begin
  dest := source;
...
begin
  ...
  if SpecialTypes(source.VType) then
    begin
...
function simpleHashTypes(VT: Integer): Boolean;
begin
  Result := False;
  case VT of
    vtInteger,
...
function hashCode(const obj : DObject) : Integer;
begin
  if simpleHashTypes(obj.VType) then
    begin
...
function DRedBlackTree.insertAt : Boolean;
begin
  {$IFDEF FPC}Result := False;{$ENDIF}
  raise DException.Create('Can''t insert to iterator in tree.');
end;

function DRedBlackTree.insertIn(_start : DIterator) : Boolean;
var pair : Dpair;
begin
  Result := true;
...
begin
  ...
  case obj1.VType of
      ...
      {$IFDEF FPC}
      vtPointer:
        Result := obj1.VPointer - obj2.VPointer;
      vtPChar:
        Result := obj1.VPChar - obj2.VPChar;
      vtPWideChar:
        Result := obj1.VPWideChar - obj2.VPWideChar;
      vtObject:
        Result := Pointer(obj1.VObject) - Pointer(obj2.VObject);
      vtClass:
        Result := Pointer(obj1.VClass) - Pointer(obj2.VClass);
      vtInterface:
        Result := Pointer(obj1.VInterface) - Pointer(obj2.VInterface);
      {$ELSE FPC}
      vtPointer:
      ...
      {$IFDEF USELONGWORD}
      vtInt64:
        Result := obj1.VInt64^ - obj2.VInt64^;
      {$ENDIF}
  end;
...
begin
  if cap <> amount then
    begin
      SetLength(items, amount);
      for i := len to amount -1 do
        InitDObject(items[i]);
      if amount < len then
        len := amount;
      cap := amount;
...
begin
  L := len;
  setSize(len + count);
  for i := L - 1 downto index + count do
    ...
    begin
      ...
      if comparator(_at(i)^, obj) <> 0 then
        begin
          ...
          found := True;
      Inc(i);
      if found and (i = _end) then
        ClearDObject(_at(i - 1)^);
    end;
  setSize(j);
end;
...
function ThisContainerAdapter(VT: Integer): Boolean;
begin
  Result := False;
  case VT of
    vtAnsiString,
...
begin
  Assert(ThisContainerAdapter(obj.vtype), 'Can only put strings into this container adapter.');
  if iterator.position = FStrings.Count then
...
begin
  ...
  repeat
    ...
    if atEnd(result) then
      break;
    comp := compare(getRef(result)^, obj);
    if comp = 0 then
      break
    else if comp < 0 then
      _start := advanceF(result)
    else
      _end := result
  until false;
end;
...
//
////////////////////////////////////////////////////////////////////
procedure _remove(container : DContainer; const obj : DObject);
begin
  ...
  begin
    ...
    try
      ...
      try
        ...
        if stable then
          m3arr.MergeSort(comparator)
        else
          m3arr.QuickSort(comparator);

        i := 0;
        iter := _start;
        while not equals(iter, _end) do
          begin
            ClearDObject(getRef(iter)^);
            New(p);
            // because of deletion the array a
            CopyDObject(m3arr.items[i]^, p^);
            putRef(iter, p^);
            Dispose(p);
            inc(i);
            advance(iter);
  ...
  case obj.VType of
      ...
      vtPWideChar:
        if obj.VPWideChar = nil then
          ...
          {$IFDEF FPC}
          WideCharToStrVar(obj.vpwidechar, result);
          {$ELSE FPC}
      ...
      vtWideChar:
        {$IFDEF FPC}
        WideCharLenToStrVar(@obj.VWideChar, 1, result);
        {$ELSE FPC}
      ...
      vtWideString:
        {$IFDEF FPC}
        WideCharToStrVar(obj.VWideString, result);
        {$ELSE FPC}
